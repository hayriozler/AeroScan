...
public sealed class Bsm(IReadOnlyDictionary<string, Element> elementMap)
    : BsmBase<TextMessageDepartureBagDto>(null, elementMap)
{
    private TextMessageDepartureBagDto ToCreateDepartureBaggageDto(long messageId, ElementResult[] elementResults)
    {
        ...
        if (sourceIndicator is null)
            throw new InvalidOperationException($"BSM {messageId}: ElementV (SourceIndicator) is required but missing");

        return new TextMessageDepartureBagDto(
            messageId,
    ...
    protected override TextMessageDepartureBagDto Handle(long messageId, ElementResult[] elementResults)
        => ToCreateDepartureBaggageDto(messageId, elementResults);
}
